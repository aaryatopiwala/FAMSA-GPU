...
static bool g_streams_inited = false;
static std::vector<cudaStream_t> g_streams;
static std::vector<cudaEvent_t> g_events;
static std::vector<cudaEvent_t> g_h2d_start;
static std::vector<cudaEvent_t> g_h2d_stop;
static std::vector<cudaEvent_t> g_kernel_start;
static std::vector<cudaEvent_t> g_kernel_stop;
static std::vector<cudaEvent_t> g_d2h_start;
static std::vector<cudaEvent_t> g_d2h_stop;

static bool ensureGpuStreams(int nStreams) {
    cudaError_t err;
    if (g_streams_inited && (int)g_streams.size() == nStreams) return true;
    // destroy existing if sizes differ
    if (g_streams_inited) {
        for (size_t i = 0; i < g_streams.size(); ++i) {
            cudaStreamDestroy(g_streams[i]);
            cudaEventDestroy(g_events[i]);
            cudaEventDestroy(g_h2d_start[i]);
            cudaEventDestroy(g_h2d_stop[i]);
            cudaEventDestroy(g_kernel_start[i]);
            cudaEventDestroy(g_kernel_stop[i]);
            cudaEventDestroy(g_d2h_start[i]);
            cudaEventDestroy(g_d2h_stop[i]);
        }
    }

    g_streams.resize(nStreams);
    g_events.resize(nStreams);
    g_h2d_start.resize(nStreams);
    g_h2d_stop.resize(nStreams);
    g_kernel_start.resize(nStreams);
    g_kernel_stop.resize(nStreams);
    g_d2h_start.resize(nStreams);
    g_d2h_stop.resize(nStreams);

    for (int i = 0; i < nStreams; ++i) {
        err = cudaStreamCreate(&g_streams[i]);
        if (err != cudaSuccess) { printCudaError("cudaStreamCreate", err); return false; }
        err = cudaEventCreateWithFlags(&g_events[i], cudaEventDisableTiming);
        if (err != cudaSuccess) { printCudaError("cudaEventCreateWithFlags", err); return false; }
        err = cudaEventCreate(&g_h2d_start[i]); if (err != cudaSuccess) { printCudaError("cudaEventCreate h2d_start", err); return false; }
        err = cudaEventCreate(&g_h2d_stop[i]); if (err != cudaSuccess) { printCudaError("cudaEventCreate h2d_stop", err); return false; }
        err = cudaEventCreate(&g_kernel_start[i]); if (err != cudaSuccess) { printCudaError("cudaEventCreate kernel_start", err); return false; }
        err = cudaEventCreate(&g_kernel_stop[i]); if (err != cudaSuccess) { printCudaError("cudaEventCreate kernel_stop", err); return false; }
        err = cudaEventCreate(&g_d2h_start[i]); if (err != cudaSuccess) { printCudaError("cudaEventCreate d2h_start", err); return false; }
        err = cudaEventCreate(&g_d2h_stop[i]); if (err != cudaSuccess) { printCudaError("cudaEventCreate d2h_stop", err); return false; }
    }
    g_streams_inited = true;
    return true;
}

// Track last ref to avoid copying bitmasks when ref unchanged
static const uint64_t* g_last_ref_bitmasks_ptr = nullptr;
static size_t g_last_bv_len = 0;
		...
		CLCSBP& lcsbp)
{
    ...
    if (!ensureGpuStreams(nStreams)) {
        fprintf(stderr, "GPU_ERROR: ensureGpuStreams failed\n");
        return;
    }
    ...
    for (int start = 0; start < n_seqs; start += BATCH_SIZE) {
        ...
        cudaStream_t currStream = g_streams[stream_idx];
        ...
        for (int i = 0; i < batch_n; ++i) {
            ...
            // copy directly into pinned buffer to avoid temporary vector and extra memcpy
            memcpy(h_concat_seqs_pinned + running_offset, pseq->data, len * sizeof(symbol_t));
            running_offset += len;
        }
        ...
        if (stream_busy[stream_idx]) {
            ...
            cudaEventSynchronize(g_events[stream_idx]);
            ...
            cudaEventElapsedTime(&ms_h2d, g_h2d_start[stream_idx], g_h2d_stop[stream_idx]);
            cudaEventElapsedTime(&ms_kernel, g_kernel_start[stream_idx], g_kernel_stop[stream_idx]);
            cudaEventElapsedTime(&ms_d2h, g_d2h_start[stream_idx], g_d2h_stop[stream_idx]);
        ...
        size_t concat_size = running_offset * sizeof(symbol_t);
        ...
        cudaEventRecord(g_h2d_start[stream_idx], currStream);
        err = cudaMemcpyAsync(d_concat_seqs, h_concat_seqs_pinned, running_offset * sizeof(symbol_t), cudaMemcpyHostToDevice, currStream);
        ...
        cudaEventRecord(g_h2d_stop[stream_idx], currStream);
        ...
        // asynch
        cudaEventRecord(g_kernel_start[stream_idx], currStream);
        ...
        cudaEventRecord(g_kernel_stop[stream_idx], currStream);
        ...
        cudaEventRecord(g_d2h_start[stream_idx], currStream);
        err = cudaMemcpyAsync(h_out_lcs_pinned, d_out_lcs, batch_n * sizeof(uint32_t), cudaMemcpyDeviceToHost, currStream);
        if (err != cudaSuccess) {
            fprintf(stderr, "GPU_ERROR: %s (%s)\n", cudaGetErrorString(err), cudaGetErrorName(err));
            return;
        }
        cudaEventRecord(g_d2h_stop[stream_idx], currStream);

        cudaEventRecord(g_events[stream_idx], currStream);
    ...
    for (int i = 0; i < nStreams; ++i) {
        if (stream_busy[i]) {
            cudaEventSynchronize(g_events[i]);
            ...
            float ms_h2d = 0, ms_kernel = 0, ms_d2h = 0;
            cudaEventElapsedTime(&ms_h2d, g_h2d_start[i], g_h2d_stop[i]);
            cudaEventElapsedTime(&ms_kernel, g_kernel_start[i], g_kernel_stop[i]);
            cudaEventElapsedTime(&ms_d2h, g_d2h_start[i], g_d2h_stop[i]);
            //printf("Stream %d batch %d: H2D=%.2f ms, Kernel=%.2f ms, D2H=%.2f ms\n", i, stream_batch_start[i] / BATCH_SIZE, ms_h2d, ms_kernel, ms_d2h);
        }
    }

    // persistent streams/events and pools are kept allocated across calls to reuse resources
